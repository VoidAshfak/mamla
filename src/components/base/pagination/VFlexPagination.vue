...
<template>
    <VFlex role="navigation" class="flex-pagination pagination is-rounded" aria-label="pagination"
        ...
        <ul class="pagination-list">
            ...
            <li>
                <RouterLink :to="paginatedLink(1)" tabindex="0" class="pagination-link"
                    ...
                    :class="[currentPage === 1 && 'is-current']" @keydown.space.prevent="(e: MouseEvent) => (e.target as HTMLAnchorElement).click()
            ...
            <li v-for="page in pages" :key="page">
                <RouterLink :to="paginatedLink(page)" tabindex="0" class="pagination-link"
                    ...
                    :class="[currentPage === page && 'is-current']" @keydown.space.prevent="(e: MouseEvent) => (e.target as HTMLAnchorElement).click()
            ...
            <li v-if="showLastLink">
                <RouterLink :to="paginatedLink(lastPage)" tabindex="0" class="pagination-link" :aria-label="t('components.v-flex-pagination.goto-page-title', { page: lastPage })
                    " :class="[currentPage === lastPage && 'is-current']" @keydown.space.prevent="(e: MouseEvent) => (e.target as HTMLAnchorElement).click()
        ...
        <RouterLink :to="paginatedLink(currentPage - 1)" tabindex="0" class="pagination-previous has-chevron"
            ...
            <i aria-hidden="true" class="iconify ltr-hidden" data-icon="feather:chevron-right" />
        </RouterLink>
        <RouterLink :to="paginatedLink(currentPage + 1)" tabindex="0" class="pagination-next has-chevron"
            @keydown.space.prevent="(e: MouseEvent) => (e.target as HTMLAnchorElement).click()"
            @click="(e: MouseEvent) => handleLinkClick(e, currentPage + 1)">
            <i aria-hidden="true" class="iconify rtl-hidden" data-icon="feather:chevron-right" />
